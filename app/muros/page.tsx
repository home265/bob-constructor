...
import { loadDefaults, loadMortars, loadWallCoefficients, loadWallOptions } from "@/lib/data/catalogs";
import { computeMuros } from "@/lib/calc/muros";
import type { Mortar, WallCoefficient, WallFormInput, WallOptions, WallResult } from "@/lib/types";
...
export default function PageMuros() {
  ...
  const [vanos, setVanos] = useState([{ lv: 0, hv: 0 }, { lv: 0, hv: 0 }, { lv: 0, hv: 0 }]);
  ...
  const { register, handleSubmit, watch, setValue, formState: { isSubmitting } } = useForm<FormValues>({
    ...
    defaultValues: { tipoMuroId: "simple", L: 0, H: 0, SA: 0, juntaMm: 10, desperdicioPct: 7 } as any
  });

  useEffect(() => {
    (async () => {
      const [o, c, m, d] = await Promise.all([
        loadWallOptions(), loadWallCoefficients(), loadMortars(), loadDefaults()
      ]);
      setOpts(o); setCoeffs(c); setMortars(m); setDefaults(d);
      // defaults: set mortero asiento al primero disponible
      if (o.morteros_asiento_ids?.length) setValue("morteroAsientoId", o.morteros_asiento_ids[0] as any);
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="grid md:grid-cols-3 gap-4">
          <label className="flex flex-col gap-1 text-sm">
            ...
            <select className="rounded border px-3 py-2" {...register("tipoMuroId")}>
              {opts.tipo_muro.map(t => <option key={t.id} value={t.id}>{t.label}</option>)}
            </select>
          </label>
          ...
          <label className="flex flex-col gap-1 text-sm">
            ...
            <select className="rounded border px-3 py-2" {...register("ladrilloId")}>
              <optgroup label="Comunes">
                {opts.ladrillos_bloques.filter(l => l.familia === "comun").map(l => <option key={l.id} value={l.id}>{l.label}</option>)}
              </optgroup>
              <optgroup label="Huecos">
                {opts.ladrillos_bloques.filter(l => l.familia === "hueco").map(l => <option key={l.id} value={l.id}>{l.label}</option>)}
              </optgroup>
              <optgroup label="Bloques cerámicos portantes">
                {opts.ladrillos_bloques.filter(l => l.familia === "ceramico_portante").map(l => <option key={l.id} value={l.id}>{l.id === "bloque_cer_27x19x20" ? "Bloque cerámico portante 27×19×20" : l.label}</option>)}
              </optgroup>
              <optgroup label="Bloques de cemento portantes">
                {opts.ladrillos_bloques.filter(l => l.familia === "hormigon").map(l => <option key={l.id} value={l.id}>{l.label}</option>)}
              </optgroup>
            </select>
          </label>
          ...
          <label className="flex flex-col gap-1 text-sm">
            ...
            <select className="rounded border px-3 py-2" {...register("juntaMm", { valueAsNumber: true })}>
              {opts.juntas_mm.map(j => <option key={j} value={j}>{j} mm</option>)}
            </select>
          </label>

          <label className="flex flex-col gap-1 text-sm">
            <span className="font-medium">Mortero de asiento</span>
            <select className="rounded border px-3 py-2" {...register("morteroAsientoId")}>
              {(opts.morteros_asiento_ids || []).map(id => <option key={id} value={id}>{id}</option>)}
            </select>
          </label>

          <NumberWithUnit label="Longitud (L)" name="L" unit="m" value={watch("L") ?? 0} onChange={(v) => setValue("L", v)} />
          <NumberWithUnit label="Altura (H)" name="H" unit="m" value={watch("H") ?? 0} onChange={(v) => setValue("H", v)} />
          <NumberWithUnit label="Superficie adicional (SA)" name="SA" unit="m²" value={watch("SA") ?? 0} onChange={(v) => setValue("SA", v)} />
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <input
            ...
            onChange={(e) => setValue("desperdicioPct", parseInt(e.target.value))}
          />
